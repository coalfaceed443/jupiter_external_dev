...
namespace CRM.admin.Package
{
    ...
    {
        protected CRM.Code.Models.CRM_Package Entity = null;

        protected void Page_Load(object sender, EventArgs e)
        {
            RunSecurity(CRM.Code.Models.Admin.AllowedSections.NotSet);
            Entity = db.CRM_Packages.SingleOrDefault(c => c.ID.ToString() == Request.QueryString["id"]);
        ...
        protected void SaveRecord(bool newRecord)
        {
            ...
            if (newRecord)
            {
                Entity = new CRM_Package();
                db.CRM_Packages.InsertOnSubmit(Entity);
            }
