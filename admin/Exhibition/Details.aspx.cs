...
namespace CRM.admin.Exhibition
{
    ...
    {
        protected CRM.Code.Models.CRM_Exhibition Entity = null;

        protected void Page_Load(object sender, EventArgs e)
        {
            RunSecurity(CRM.Code.Models.Admin.AllowedSections.NotSet);
            Entity = db.CRM_Exhibitions.SingleOrDefault(c => c.ID.ToString() == Request.QueryString["id"]);
        ...
        protected void SaveRecord(bool newRecord)
        {
            ...
            if (newRecord)
            {
                Entity = new CRM_Exhibition();
                Entity.IsArchived = false;
                db.CRM_Exhibitions.InsertOnSubmit(Entity);
            }
