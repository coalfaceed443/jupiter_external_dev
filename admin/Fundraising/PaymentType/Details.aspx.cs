...
namespace CRM.admin.Fundraising.PaymentType
{
    ...
    {
        protected CRM.Code.Models.CRM_PaymentType Entity = null;

        protected void Page_Load(object sender, EventArgs e)
        {
            RunSecurity(CRM.Code.Models.Admin.AllowedSections.NotSet);
            Entity = db.CRM_PaymentTypes.SingleOrDefault(c => c.ID.ToString() == Request.QueryString["id"]);
        ...
        protected void SaveRecord(bool newRecord)
        {
            ...
            if (newRecord)
            {
                Entity = new CRM_PaymentType();
                db.CRM_PaymentTypes.InsertOnSubmit(Entity);
                Entity.OrderNo = Code.Utils.Ordering.Ordering.GetNextOrderID(db.CRM_PaymentTypes);
            }
