    ...
    public class StringEnum
    {
        ...
        public static string GetStringValue(Enum value)
        {
            ...
            else
            {
                ...
                if (attrs.Length > 0)
                {
                    //If two processes are running at the same time this can get called twice if execution is simultaneous.
                    //Catch the error and return the output value as it has already been added by another process
                    try
                    {
        ...
        public static string GetStringValue<TEnum>(int val) where TEnum : struct
        {
            ...
            FieldInfo fi = enumType.GetField(Enum.GetName(enumType, val));
            StringValueAttribute[] attrs = fi.GetCustomAttributes(typeof(StringValueAttribute), false) as StringValueAttribute[];
            if (attrs.Length > 0)
            {
                output = attrs[0].Value;
